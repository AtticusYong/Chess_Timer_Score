...
const timesUp = new Audio('audio/times-up.wav');
...
const timeWarning = (player, min, sec) => {
    ...
    if (min < 1 && sec <= 30) {
        if (player === 1) {
            document.querySelector('.player-1 .player__digits').style.color = 'red';
        } else {
            document.querySelector('.player-2 .player__digits').style.color = 'red';
        }
    }
}
...
document.getElementById("countingOne").innerText = playerOne;
document.getElementById("countingTwo").innerText = playerTwo;
